...
//		return lavoratore.handleItemAdd(item);
		...
		if (esito)
			System.out.println(lavoratore.getNome() + "(" + lavoratore.getQualifica() + ") ha aggiunto l'articolo " + item.getProdotto());
		else
			System.out.println(lavoratore.getNome() + "(" + lavoratore.getQualifica() + ") non può aggiungere articoli");
		return esito;
	}
...
//		return lavoratore.handleItemRemove(item);
		...
		for (Item it : items) {
			if (item.equals(it)) {
				...
				if (esito)
					System.out.println(lavoratore.getNome() + "(" + lavoratore.getQualifica() + ") ha rimosso l'articolo " + item.getProdotto());
				else
					System.out.println(lavoratore.getNome() + "(" + lavoratore.getQualifica() + ") non può rimuovere articoli");
				return esito;
			}
		}
